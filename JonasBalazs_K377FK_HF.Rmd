...
lm.fwd.all <- stepAIC(lm.min,
                  ...
                  scope=list(lower=lm.min,
                             upper=~ Accepted + Announced + BidAmount +
                               ...
                               YieldSpread))
summary(lm.fwd.all)
names(lm.fwd.all$coefficients)

```

A továbbiakban csak azokat hagyom a változók között, amik nincsenek determinisztikus vagy más erős kapcsolatban a független változóval.
...
Az AIC értékeken az látszik, hogy amint elhagyom a determinisztikus (árfolyamok) és erősen korreláló tagokat az illeszkedés kevésbé pontos. Az mRMR ugyanerre az eredményre vezetne.
...
A forward módszerhez képest mind az átlagos négyzetes hiba, mind az AIC alapján jobban teljesít a ridge módszer. Meg kell jegyezni, hogy a $R^2$ még továbbra is nagyon magas. A *glmnet* cross validation függvénye az átlagos négyzetes hibát minimalizálta, amit viszonylag alacsony $\lambda$-val ért el (pl a $X'X$ mátrix normájához képest, ami `r norm(t(X)%*%X)`), vagyis kicsit tért csak el lineáris regresszióhoz képest.
...
`r rse(lm.fwd.all$residuals, length(lm.fwd.all$coefficients))` &
`r aic(lm.fwd.all$residuals, length(lm.fwd.all$coefficients))` &
`r r.squared(lm.fwd.all$residuals, Y, length(lm.fwd.all$coefficients)) # az elvárt érték minden esetben ugyanaz`\\
\hline
