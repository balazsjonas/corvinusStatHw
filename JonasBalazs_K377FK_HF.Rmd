...
```{r dependencies, message=TRUE, warning=FALSE, include=FALSE}
...
pacman::p_load(ggplot2,
               ...
               tidyr, # replace_na
               dplyr, # select
               psych, # descibe, winsor.mean, winsor.sd
...
```{r loading.excel, warning=FALSE, include=FALSE}
...
```{r bond5.filter, include=FALSE}
bond5 <- issues[issues$Tenor == "5Y",
                c( "Date", 
                  "Accepted (mln)", "Avg Yield (%)", # vizsgált mennyiségek
                  "Min Yield (%)", "Max Yield (%)", #
...
bond5$YieldSpread <- bond5$MaxYield - bond5$MinYield

# Sikeres kibocsátások:
bond5 <- bond5[bond5$AcceptedAmount>0, ]
```
...
ggplot(data=bond5, aes(x=Yield)) +
  ...
  labs(title="Átlaghozamok az 5 éves kötvények elsődleges kibocsátásakor",
       x="Átlaghozam (%)",
       y= "Gyakoriság (db)")
...
```{r hist}
descAmount <- describe(bond5$AcceptedAmount, quant=c(0.25, 0.5, 0.75))
print(t(descAmount))
...
print(paste("Outlierek száma: ",
  sum(bond5$AcceptedAmount>mean(bond5$AcceptedAmount) + 1.5*IQR(bond5$AcceptedAmount))))

print("A kiugró értékek dátumai:")
bond5[bond5$AcceptedAmount>mean(bond5$AcceptedAmount) + 1.5*IQR(bond5$AcceptedAmount), "Date"]
```
...
Az Államadósságkezelő igyekszik minél olcsóbban finanszírozni az országot, ami az elfogadott mennyiség és hozam közötti erős negatív korrelációban látszik. Ezt a kapcsolatot kicsit pontosabban is megvizsgálom.
...
## TODO ezt sokkal előbbre
```{r linreg}
lm1 <- lm(AcceptedAmount ~ Yield + MinYield + MaxYield +
            MinPrice + AvgPrice + MaxPrice +
            YieldSpread,
          data=bond5)
summary(lm1)

lm2 <- lm(AcceptedAmount ~ YieldSpread + Yield, data=bond5)
summary(lm2)
...
# 4. Egymintás hipotézisvizsgálat
