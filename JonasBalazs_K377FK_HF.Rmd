...
## bimodális és 5y ammount vs yield

## 1 mintás t test, korreláció

## outlier, winsorized mean

## bimodális kernel estimator

## 5y amoutn vs yield --> knn?
...
colnames(bond5) <- c("Date", "Yield", "Accepted",
                     "MinYield", "MaxYield",
                     "Announced", "BidAmount", "AvgPrice", "MinPrice", "MaxPrice",
                     "YieldSpread", "Period")
...
A következőkben a két vizsgált mennyiséget az elfogadott mennyiség és hozamot mutatom be, majd ezek kapcsolatát.
...
Először az időszak, mint kategórikus változó és dátum nélkül.
...
Az Államadósságkezelő igyekszik minél olcsóbban finanszírozni az országot, ami az elfogadott mennyiség és hozam közötti erős negatív korrelációban látszik. Nem meglepő a meghirdetett és elfogadott mennyiségek közötti erős korreláció sem.

```{r corr, cache=cache.enabled}
print(cor(select(bond5,-Period, -Date )), digits = 1)
corPlot(select(bond5,-Period, -Date ), FALSE)
```

## Átlaghozam vs elfogadott mennyiség


Az Államadósság kezelő által befolyásolható mennyiség hozam és az elfogadott mennyiség. Mint korábban láttuk a kettő között gyenge negatív korreláció van. Ez azért meglepő, mert az ÁKK a legkisebb hozamútól kezdve sorban fogadja el az egyre nagyobb hozamú (kisebb árfolyamú) ajánlatokat a stratégiája által meghatározott mértékig. Magyarázat lehet, hogy különböző aukciókra különböző ajánlatok érkeznek
...
bond5sampled <- sample_n(bond5, 200)
plot(scale(bond5sampled))
bond5sampled <- bond5sampled[,c('Accepted', 'Yield')]
fifa_dist<-dist(bond5sampled, method="euclidean") # kiszámoljuk a mintába került elemek közötti távolságot
heatmap(as.matrix(fifa_dist)) # hőtérképet rajzolunk
```
<!-- A két változó között gyenge negatív kapcsolat van. A Pearson korrelációhoz tartozó p érték alapján elvethetjük, hogy a lineáris korreláció nulla lenne. A Spearman korrelációnál hasonló eredményt kapunk. -->

<!-- ```{r corr2} -->
<!-- cor(select(bond5,-Period, -Date )) -->
<!-- corPlot(select(bond5, -Period, -Date)) -->
<!-- ggplot(bond5, aes(x=Yield, y=AcceptedAmount)) + -->
<!--   geom_point() + -->
<!--   geom_smooth(method=lm) -->

<!-- cor.test(bond5$Yield, bond5$AcceptedAmount, method='pearson') -->
<!-- cor.test(bond5$Yield, bond5$AcceptedAmount, method='spearman', exact = FALSE) -->
...
```{r linreg}
lm1 <- lm(Yield ~ Accepted + Announced + BidAmount +
...
```
# TODO változószelekció

## TODO melyiket tartsuk meg.
